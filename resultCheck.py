"""
resultCheck.py — compute accuracy and F1 score from results2.csv, printing every step.
...
# ── accuracy (mirrors evaluation.py compute_correct, adapted for results2.csv) ─

def compute_correct_row(row):
    """Return 1/0/None using the same logic as evaluation.py compute_correct.

    Reads "support [true value]" (results2.csv column name).
...
def accuracy_for_strategy(df: pd.DataFrame) -> tuple[int, int]:
...
def print_strategy(strategy: str, df: pd.DataFrame):
    ...
    for cls in classes:
        ...
        total_check = m["TP"] + m["FP"] + m["FN"] + m["TN"]
        ...
        print(f"  {cls:14s} {m['TP']:>5} {m['FP']:>5} {m['FN']:>5} {m['TN']:>5}  "
              ...
              f"  [2·{m['TP']}/{denom_check}]  {check_str}")
    if all_sums_ok:
...
def main():
    ...
    # ── top-level accuracy summary (mirrors evaluation.py output) ─────────
